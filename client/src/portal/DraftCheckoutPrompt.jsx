...
/**
 * Test-only sheet: shown when checkout returns `mode: draft_hubtel` (or legacy draft_momo).
 */
export function DraftCheckoutPrompt({ open, payload, onClose, onComplete }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="draft-checkout-title"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-amber-500/20 pb-3">
          ...
          <div>
            <p id="draft-checkout-title" className="text-sm font-semibold text-amber-100">
              Hubtel (draft)
            </p>
            <p className="text-xs text-slate-500">Test — no live Hubtel call</p>
          </div>
        </div>
        ...
        <p className="mt-3 text-center text-[11px] leading-relaxed text-slate-500">
          No real charge. Tap below to simulate approving payment.
        </p>
        ...
        <div className="mt-5 flex flex-col gap-2">
          ...
          >
            {busy ? 'Processing…' : 'Simulate approve'}
          </button>
          <button
            type="button"
            disabled={busy}
...
/** @deprecated use DraftCheckoutPrompt */
export const DraftMomoPrompt = DraftCheckoutPrompt;
